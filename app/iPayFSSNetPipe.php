...
class iPayFSSNetPipe
{
	...
	//added by pavithran for authRespCode
	...
	function performPaymentInitializationHTTP()
	{
		...
		try {
			...
			//echo "$request";
			$this->webAddress = $this->webAddress . "/PaymentHTTP.htm?param=paymentInit" . $request;
			//	dd($this->webAddress);
		} catch (Exception $e) {
	...
	{
		$contentType = "";
		//$webAddress="http://10.44.71.191:8090/IPAYOAB";
	...
	function performTransactionHTTP()
	{
		...
		try {
			...
			$this->webAddress = $xmlData['webaddress'];
			//$this->webAddress = "http://10.44.71.241:8089/iPayFSSNET/"; //test
			$this->setid($xmlData['id'] . "");
