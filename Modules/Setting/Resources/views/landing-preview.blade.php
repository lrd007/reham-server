<html>
    ...
    <body>
        ...
        <div class="container">
            <div class="row">
                <div class="col-sm-6">
                    <h3 class="mt-4">{{ setting('landing_page_contact_heading' . withLocalization()) }}</h3>
                    @if(setting('landing_page_contact_type') == 'image' && !empty(setting('landing_page_contact_image')))
                        @if(setting('landing_page_contact_image_url'))
                            <a href="{{ url(setting('landing_page_contact_image_url')) }}" target="_blank">
                                <img src="{{ url(uploads_files('landing_page', null,true) . '/' . setting('landing_page_contact_image')) }}" alt="" height="300px">
                            </a>
                        @else                        
                            <img src="{{ url(uploads_files('landing_page', null,true) . '/' . setting('landing_page_contact_image')) }}" alt="" height="300px">
                        @endif
                    @elseif(setting('landing_page_contact_type') == 'video' && !empty(setting('landing_page_contact_video')))
                        <video class="mb-5" width="450" height="250" controls>
                            <source src="{{ url(uploads_files('landing_page', null,true) . '/' . setting('landing_page_contact_video')) }}" type="video/mp4">
                            Your browser does not support the video tag.
                        </video>
                    @else
                        {!! setting('landing_page_contact_description' . withLocalization()) !!}
                        @if(!empty(setting('landing_page_contact_image_url')))
                            <a href="{{ url(setting('landing_page_contact_image_url')) }}" target="_blank">{{ __('Please click here for more details') }}</a>
                        @endif
                    @endif
                </div>
                <div class="col-sm-6">
                    <div class="card card-body text-center mt-4 mb-4" style="background-color: #ea8bb8;">
                        <h4 class="card-title">{{ __('Stay In Touch With Us') }}</h4>

                        <p class="card-text px-5">
                            <input type="text" placeholder="{{ __('Email') }}" class="form-control" style="border-radius: 50px;">
                        </p>
                        <p class="card-text">
                            <button class="btn btn-default waves-effect waves-light" style="border-radius: 50px;border: 1px solid white;">{{ __('Subscribe') }}</button>
                        </p>
                    </div>
                </div>
            </div>            
